...
//    @GetMapping("/my/accept")
    ...
    public R accept(@RequestBody Map<String, Object> json, @RequestHeader(value = "token") String token) {
        ...
        // 查询当前是否有任务在进行中
        UserQuest current = userQuestService.getOne(new QueryWrapper<UserQuest>().eq("user_id", userId).eq("quest_state", 0).or().eq("quest_state", 1).last("limit 1")); // 0为未开始的任务，1为进行中的任务，均为当前任务
        if (current == null) { // 判断是否存在当前任务
            // 新建用户任务并设置内容
...
//    public R complete(int userQuestId, String endTimeStr) {
     ...
     */
//    @GetMapping("/giveUp")
//    public R giveUp(int id, String endTimeStr) {
     ...
     * 用于：微信小程序首页
     */
//    @GetMapping("/findCurrentQuest")
//    public R findCurrent(Integer userId){
